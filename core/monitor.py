...
def start_monitoring(packages, intent_url, timeout_seconds, max_retries, cooldown_secs, stats=None, config_data=None):
    ...
    # PHASE 2: bagikan referensi `stats` (dict, mutable) ke agent_client supaya
    # heartbeat ke Joki Control Bot melaporkan status package yang sebenarnya,
    # bukan kosong. Aditif & non-fatal -- kalau modul tidak ada/gagal, watchdog
    # tetap jalan seperti biasa (fitur agent opsional, lihat agent_client.py).
    try:
        from core.agent_client import set_stats_reference
        set_stats_reference(stats)
    except Exception:
        pass
